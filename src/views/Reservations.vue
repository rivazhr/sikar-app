...
<template>
  <div class="flex-1">
    <div class="flex">
      ...
      <button @click="showDialog = true" class="bg-primary opacity-90 hover:opacity-100 text-white font-bold py-2 px-4 rounded mb-4">
        Add Reservation
      </button>
    </div>
    ...
    <div v-if="showDialog" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
      <div class="bg-white rounded-lg shadow-lg p-6 w-4/5 sm:w-2/5 text-start">
        ...
        <form @submit.prevent="addReservation">
          ...
          <div class="mb-4">
            <label for="vehicle_category" class="block text-sm font-medium text-gray-700">Vehicle Category</label>
            <select v-model="newReservation.vehicle_category" id="vehicle_category" class="mt-1 p-2 w-full border bg-gray-100 text-black border-gray-300 rounded-md">
